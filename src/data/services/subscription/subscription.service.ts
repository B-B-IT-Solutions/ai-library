...
export class SubscriptionService {
  ...
  async getPlanByTier(tier: DSubscriptionTier): Promise<DSubscriptionPlan | null> {
  ...
  async createCheckoutSession(params: {
    ...
    const checkoutSession = await stripe.checkout.sessions.create({
      ...
      success_url:
        params.successUrl || `${APP_URL}/subscription/success?session_id={CHECKOUT_SESSION_ID}`,
  ...
  async createPortalSession(userId: string): Promise<{ url: string }> {
    const user = await this.subscriptionRepo.pGetUserByStripeCustomerId(
      userId
    );
  ...
  async handleSubscriptionUpdated(
    ...
    // Update subscription
    await this.subscriptionRepo.pUpdateSubscription(localSubscription.userId, {
      status: newStatus,
      currentPeriodStart: new Date(stripeSubscription.current_period_start * 1000),
      currentPeriodEnd: new Date(stripeSubscription.current_period_end * 1000),
