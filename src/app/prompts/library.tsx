...
const PromptManager = () => {
  ...
  const saveData = async (newPrompts, newCategories, newFavorites) => {
    try {
      ...
      if (newFavorites !== undefined) {
        await window.storage.set("favorites-data", JSON.stringify(newFavorites));
      }
    } catch (error) {
  ...
  const startEdit = () => {
    ...
    setFormData({
      ...
      content: selectedPrompt.versions[selectedPrompt.versions.length - 1].content,
  ...
  const filteredPrompts = prompts.filter((p) => {
    const matchesSearch =
      ...
      p.versions.some((v) => v.content.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const filteredTemplates = PREDEFINED_PROMPTS.filter((template) => {
    ...
    const matchesCategory =
      templateCategory === "all" || template.categories.includes(templateCategory);
    return matchesSearch && matchesCategory;
  });
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex">
      ...
      <div className="w-64 bg-white border-r border-slate-200 flex flex-col">
        <div className="p-6 border-b border-slate-200">
          <h1 className="text-2xl font-bold text-slate-900">Prompt Manager</h1>
        </div>
          ...
          {activeMenu === "prompts" && (
            <>
              <header className="mb-8">
                <h2 className="text-3xl font-bold text-slate-900 mb-2">All Prompts</h2>
                    ...
                    <div className="bg-white rounded-lg p-6 border border-slate-200 shadow-sm">
                      ...
                      {selectedPrompt.followUpPrompts &&
                        selectedPrompt.followUpPrompts.length > 0 && (
                          <div className="mb-6">
                            <button
                              onClick={() => togglePromptContent("followups")}
                              className="w-full flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-200 hover:bg-slate-100 transition-colors"
                            >
                              <span className="font-semibold text-slate-900 flex items-center gap-2">
                                Follow-up Prompts ({selectedPrompt.followUpPrompts.length}
                                )
                              </span>
                            ...
                            {expandedPromptContent["followups"] && (
                              <div className="mt-2 space-y-2">
                                {selectedPrompt.followUpPrompts.map((followUp, idx) => (
                                  <div
                                    key={idx}
                                  ...
                                  >
                                    <div className="flex items-start gap-2">
                                      ...
                                      <button
                                        onClick={() =>
                                          copyToClipboard(followUp, `followup-${idx}`)
                                        }
                      ...
                      <div className="space-y-4">
                        ...
                        {[...selectedPrompt.versions].reverse().map((version, idx) => (
                          <div
                            key={version.version}
          ...
          {activeMenu === "favorites" && (
            <>
              <header className="mb-8">
                ...
                <p className="text-slate-600">Your starred prompts for quick access</p>
              </header>
                  ...
                  <div className="bg-white rounded-lg border border-slate-200 shadow-sm max-h-[600px] overflow-y-auto">
                    ...
                    {favoritePrompts.length > 0 ? (
                      ...
                      <div className="p-12 text-center text-slate-500">
                        ...
                        <p className="text-sm mt-2">Star prompts to add them here</p>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-200">
                        <div>
                          <p className="font-medium text-slate-900">Total Prompts</p>
                  ...
                  <div>
                    ...
                    <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                      <p className="text-sm text-slate-600 mb-2">
                        Prompt Manager helps you organize, version, and manage your AI
                        prompts efficiently.
                      </p>
